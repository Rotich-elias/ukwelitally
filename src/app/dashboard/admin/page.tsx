...
export default function AdminDashboard() {
  ...
  const handleCreateBallotCandidate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(editingBallot ? 'Ballot candidate updated successfully!' : 'Ballot candidate created successfully!')
      setBallotFormData({
        full_name: '',
        position: 'president',
        party_name: '',
      ...
      setError(err.message || 'Failed to save ballot candidate')
    }
  }
                ...
                <tbody className="divide-y divide-dark-700">
                  {loading ? (
                    ...
                    users.map((user) => (
                      ...
                      <td className="px-6 py-4">
                        {user.role === 'candidate' && user.position ? (
                ...
                {ballotFormData.position !== 'president' && (
                  <div className="border border-dark-700 rounded-lg p-4 bg-dark-900/30">
                    ...
                    <p className="text-xs text-dark-400 mb-3">
                      {ballotFormData.position === 'governor' || ballotFormData.position === 'senator' || ballotFormData.position === 'women_rep'
                        ? 'Select county for this candidate'
                        : ballotFormData.position === 'mp'
                        ? 'Select constituency for this candidate'
                        : 'Select ward for this candidate (MCA)'}
                    </p>
                    <LocationSelector
                      initialValues={{
